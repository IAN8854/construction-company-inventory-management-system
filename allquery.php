...
if(isset($_POST['showsalesreport'])){
    ...
    while ($row = mysqli_fetch_assoc($select)){
        ...
        $grantotal += (($rowout['totalqty']*$row['sellprice'])-($rowout['totalqty']*$row['amount']))-(($rowout['totalqty']*$row['sellprice'])-($rowout['totalqty']*$row['amount']))*($_POST['tax']/100);
        $totaltax += (($rowout['totalqty']*$row['sellprice'])-($rowout['totalqty']*$row['amount']))*($_POST['tax']/100);
    echo '<tr style="cursor: pointer">
                                ...
                                <td align="right">'. number_format((($rowout['totalqty']*$row['sellprice'])-($rowout['totalqty']*$row['amount']))*($_POST['tax']/100),2).'</td>
                                <td align="right">'. number_format((($rowout['totalqty']*$row['sellprice'])-($rowout['totalqty']*$row['amount']))-(($rowout['totalqty']*$row['sellprice'])-($rowout['totalqty']*$row['amount']))*($_POST['tax']/100),2).'</td>
                            </tr>';
    }
    echo '<tr>
        <td colspan="4"></td>
...
if(isset($_POST['showproduct'])){
    ...
    echo '<tr style="cursor: pointer">
                                ...
                                <td>Ksh.'. number_format($row['sellprice'],2).'</td>
...
<div id="chartContainer" style="height: 370px; width: 100%;"></div>
    <script>
                        var chart = new CanvasJS.Chart("chartContainer", {
                            ...
                            title: {
                                text: "Profit Growth Rate - <?php echo $_POST['myyear'] ?>"
                            },
                            axisY: {
                                title: "Growth Rate (in Profit)",
                                suffix: "",
                                includeZero: false
                            },
                            axisX: {
